...
public class RegisterUserOther extends HttpServlet {
		...
		if(session.isNew()) {
			...
			if(params.hasMoreElements()) { // existing session with parameters: user filled in form and pressed register
				...
				if(numFormFieldsFilledIn != MAX_FORM_FIELDS) {
					...
					out.println("<p>To continue your registration click <a href='/Compsci719Lab18/RegisterUser'>here</a>.</p>"); // ?continue=yes
				}
				else { // form was completely filled in, reload the RegisterUser servlet page and display values 
					response.sendRedirect("/Compsci719Lab18/RegisterUser"); // ?continue=no
	...
	protected int storeFormData(File userFile, HttpServletRequest request) {
		...
		try {
			...
			bob = new User(first, last, nick, email);
			bobby.addUser(bob);
	...
	protected void createForm(Properties formFields, PrintWriter out, boolean onlyDisplay) {
		...
		out.println("<form style='width:500px;margin:auto;' id='userform_id' name='userform' method='post' action='/Compsci719Lab18/RegisterUser'>");
